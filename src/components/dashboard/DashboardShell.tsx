...
function kpiColor(val: number, invert = false): string {
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        ...
        {displayVal !== undefined && (
          <span style={{
            ...
            {barInvert
              ? (displayVal < 40 ? "Low" : displayVal < 70 ? "Med" : "High")
              : (displayVal >= 65 ? "Good" : displayVal >= 35 ? "Mid" : "Low")}
          </span>
        )}
      </div>
      <div>
        <div style={{ fontSize: 26, fontWeight: 800, color: C.t1, fontFamily: C.fontDisplay, lineHeight: 1 }}>{value}</div>
        <div style={{ fontSize: 11, color: C.t3, marginTop: 5, fontFamily: C.fontMono, letterSpacing: "0.04em", textTransform: "uppercase" as const }}>{label}</div>
      </div>
      {displayVal !== undefined && (
        <div>
...
const TimelineTooltip = ({ active, payload }: TimelineTooltipProps) => {
  ...
  return (
    <div style={{ background: "#0d1117", border: `1px solid ${col}30`, borderRadius: 10, padding: "10px 14px", maxWidth: 240 }}>
      ...
      <p style={{ fontSize: 12, color: C.t2, marginTop: 6, lineHeight: 1.5 }}>
        {d.sentence?.slice(0, 100)}{(d.sentence?.length ?? 0) > 100 ? "…" : ""}
      </p>
...
interface RadarPayloadItem {
  payload: { emotion: string; value: number };
}
...
function TimelineDot(props: DotProps) {
  const { cx = 0, cy = 0, payload, index } = props;
  const col = (payload?.score ?? 0) > 0 ? C.cyan : (payload?.score ?? 0) < 0 ? C.red : C.amber;
  return <circle key={index} cx={cx} cy={cy} r={3.5} fill={col} stroke={C.card} strokeWidth={1.5} />;
}
...
export default function DashboardShell() {
  ...
  const {
    sentence_level_analysis: sentences,
    business_kpis: kpis,
    overall_sentiment,
    emotion_detected,
    summary,
  ...
  /* ── Chart data ── */
  const timelineData = sentences.map((s, i) => ({
    index: i + 1,
  ...
  const topEmotion = EMOTIONS.find(e => e.toLowerCase() === emotion_detected?.toLowerCase());
  if (topEmotion && emotionCounts[topEmotion] === 0) emotionCounts[topEmotion] = 2;
  ...
  const posCount = sentences.filter(s => s.sentiment === "Positive").length;
  const negCount = sentences.filter(s => s.sentiment === "Negative").length;
  const neuCount = sentences.filter(s => s.sentiment === "Neutral").length;
  const pieData = [
  ...
  const sentimentScore = Math.round(((result.overallScore ?? 0) + 1) * 50);

  const frustNorm = kpis.customer_frustration_index > 10
    ? kpis.customer_frustration_index
    : kpis.customer_frustration_index * 10;
  const empNorm = kpis.agent_empathy_score > 10
    ? kpis.agent_empathy_score
    : kpis.agent_empathy_score * 10;
  const churnNorm = kpis.churn_risk_percentage ?? (kpis.churn_risk === "High" ? 75 : kpis.churn_risk === "Medium" ? 45 : 20);
  const resProbNorm = kpis.resolution_probability ?? 75;

  const emotionBarColors = [C.purple, C.blue, C.cyan, C.amber, C.red, C.orange];
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 1440, margin: "0 auto", padding: "32px 32px 64px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1.4fr", gap: 16, marginBottom: 20 }}>
          ...
          <div style={{
            ...
            <div style={{ fontSize: 10, fontFamily: C.fontMono, letterSpacing: "0.1em", color: C.t3, textTransform: "uppercase" as const }}>Sentiment Score</div>
            <Gauge value={sentimentScore} color={sentimentCol} size={110} />
            <div style={{ fontSize: 12, color: C.t2, textAlign: "center" as const }}>
          ...
          <div style={{
            ...
            <div style={{ fontSize: 10, fontFamily: C.fontMono, letterSpacing: "0.1em", color: C.t3, textTransform: "uppercase" as const, marginBottom: 8 }}>Primary Emotion</div>
            <div>
              <div style={{ fontSize: 40, marginBottom: 8 }}>
                {["Joy","Happy"].includes(emotion_detected) ? "😊"
                  : ["Anger","Frustrated","Angry"].includes(emotion_detected) ? "😠"
                  : ["Sadness","Disappointed","Sad"].includes(emotion_detected) ? "😔"
                  : ["Fear","Anxious"].includes(emotion_detected) ? "😰"
                  : emotion_detected === "Surprise" ? "😮"
                  : ["Trust","Satisfied"].includes(emotion_detected) ? "🤝"
                  : "😐"}
              </div>
              <div style={{ fontSize: 22, fontWeight: 800, fontFamily: C.fontDisplay, color: C.t1 }}>{emotion_detected || "Neutral"}</div>
          ...
          <div style={{
            ...
            <div style={{ fontSize: 10, fontFamily: C.fontMono, letterSpacing: "0.1em", color: C.t3, textTransform: "uppercase" as const }}>Call Analytics</div>
            {[
              { label: "Sentences Analysed", val: sentences.length, col: C.blue },
              { label: "Resolution Status", val: kpis.issue_resolved ? "Resolved ✓" : "Open ✗", col: kpis.issue_resolved ? C.cyan : C.red },
              { label: "Call Duration", val: result.callDuration ? `${Math.floor(result.callDuration / 60)}m ${result.callDuration % 60}s` : "—", col: C.t2 },
            ].map(item => (
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <span style={{
                ...
                color: kpis.churn_risk === "High" ? C.red : kpis.churn_risk === "Medium" ? C.amber : C.cyan,
                background: kpis.churn_risk === "High" ? C.redDim : kpis.churn_risk === "Medium" ? C.amberDim : C.cyanDim,
                border: `1px solid ${kpis.churn_risk === "High" ? C.red : kpis.churn_risk === "Medium" ? C.amber : C.cyan}25`,
                borderRadius: 6, padding: "3px 10px",
              }}>{kpis.churn_risk ?? "Low"}</span>
            </div>
          </div>
          ...
          <div style={{
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div style={{ fontSize: 10, fontFamily: C.fontMono, letterSpacing: "0.1em", color: C.t3, textTransform: "uppercase" as const }}>AI Executive Summary</div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(6, 1fr)", gap: 14, marginBottom: 20 }}>
          <KPICard label="Customer Frustration" value={`${Math.round(frustNorm)}%`}
            sub={`${frustNorm < 40 ? "Low" : frustNorm < 70 ? "Moderate" : "High"} frustration`}
            icon="😤" color={C.red} barValue={Math.round(frustNorm)} barInvert />
          <KPICard label="Agent Empathy" value={`${Math.round(empNorm)}%`}
            sub={`${empNorm >= 65 ? "Strong" : empNorm >= 35 ? "Moderate" : "Weak"} empathy`}
            icon="💚" color={C.cyan} barValue={Math.round(empNorm)} />
          <KPICard label="Churn Risk" value={`${Math.round(churnNorm)}%`}
            sub="Customer retention signal"
            icon="📉" color={C.orange} barValue={Math.round(churnNorm)} barInvert />
          <KPICard label="Resolution Probability" value={`${Math.round(resProbNorm)}%`}
            sub="Issue closure likelihood"
            icon="✅" color={C.blue} barValue={Math.round(resProbNorm)} />
          <KPICard label="Positive Utterances" value={posCount}
            sub={`${Math.round((posCount / totalSentences) * 100)}% of conversation`}
            icon="📈" color={C.cyan} barValue={Math.round((posCount / totalSentences) * 100)} />
          <KPICard label="Interaction Quality"
            value={empNorm >= 65 && kpis.issue_resolved ? "High" : empNorm >= 40 ? "Mid" : "Low"}
            sub="CX quality composite score"
            icon="⭐" color={C.purple}
            barValue={Math.round((empNorm + (kpis.issue_resolved ? 20 : 0) + resProbNorm) / 2.2)} />
        </div>
          ...
          <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: "24px" }}>
            ...
            <div style={{ marginTop: 16 }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", marginTop: 6 }}>
                {([["Positive", posCount, C.cyan], ["Neutral", neuCount, C.amber], ["Negative", negCount, C.red]] as [string, number, string][]).map(([lbl, cnt, col]) => (
                  <span key={lbl} style={{ fontSize: 10, color: col, fontFamily: C.fontMono }}>
                    {lbl}: {cnt} ({Math.round((cnt / totalSentences) * 100)}%)
                  </span>
                ))}
              </div>
            </div>
          ...
          <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: "24px", display: "flex", flexDirection: "column" }}>
            ...
            <div style={{
              ...
              <div>
                ...
                <div style={{ fontSize: 14, fontWeight: 700, color: C.t1 }}>{emotion_detected || "Neutral"}</div>
              </div>
            </div>
              ...
              <div style={{ height: 200 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={emotionBarData} layout="vertical" margin={{ top: 0, right: 20, bottom: 0, left: 0 }}>
                    ...
                    <Tooltip formatter={(value: any) => [`${value}%`, "Intensity"]} contentStyle={{ background: "#0d1117", border: `1px solid ${C.border}`, borderRadius: 8, fontSize: 11 }} />
              ...
              <div style={{ width: 200, height: 200, flexShrink: 0 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    ...
                    <Tooltip formatter={(value: any) => [value, "sentences"]} contentStyle={{ background: "#0d1117", border: `1px solid ${C.border}`, borderRadius: 8, fontSize: 11 }} />
                  </PieChart>
                </ResponsiveContainer>
              </div>
        ...
        {/* ════════ FOOTER ════════ */}
        <div style={{ marginTop: 32, textAlign: "center" as const, borderTop: `1px solid ${C.border}`, paddingTop: 24 }}>
